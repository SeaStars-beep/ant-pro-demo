...
const Index = () => {
  const { detail, columns } = useViewModel();
  const { base, user, finance, oilStation, log = [] } = detail;
  return (
    <PageContainer title="订单详情">
      <div className={styls.detailWrap}>
        ...
        <Descriptions title="财务信息" bordered column={2} style={{ marginBottom: 12 }}>
          ...
          <Descriptions.Item label="会员红包优惠金额">
            {finance?.amountDiscountVip}
          </Descriptions.Item>
          <Descriptions.Item label="油滴抵扣">{finance?.oilDropletAmount}</Descriptions.Item>
          <Descriptions.Item label="服务费">{finance?.amountServiceCharge}</Descriptions.Item>
          ...
          <Descriptions.Item label="余额支付金额">{finance?.amountBalance}</Descriptions.Item>
        </Descriptions>
        ...
        <Table dataSource={log} columns={columns} pagination={false} />
      </div>
    </PageContainer>
  );
};
